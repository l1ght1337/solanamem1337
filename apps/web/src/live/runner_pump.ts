...
export function runBot(connection: Connection, bot: LiveBot, ctx: RunCtx) {
  ...
  async function trade(
    ...
    // риск‑профиль из стора
    let risk = {
      maxImpact: 0.010, maxDrawdown: 0.12, reserveSol: 0.0060,
      maxNotionalPerMin: 0.0008, maxBuysPerMin: 1, maxSellsPerMin: 4,
      lossThrPct: 0.003, lossWindowMs: 30000, lossCooldownMs: 180000,
      maxBuySliceSol: 0.00035, maxSellSliceTokPct: 0.035,
      minSliceGapMs: 600, maxSliceGapMs: 1800
    } as any;
    try { const r = ctx.getRisk?.(); if (r) risk = r; } catch {}

    // актуальные аллокации
    try {
      const allocUI = ctx.getAlloc?.();
    ...
    try { const s = ctx.getTradeStep?.(); if (s) step = s; } catch {}
    const pickStep = () => {
    ...
    let amountTok = side === "sell" && opts?.sellTokens ? roundTok(opts.sellTokens, decimals) : undefined;

    // коридор перед исполнением
    try {
    ...
    if (side === "buy") {
      ...
      const stepCfg = { ...(ctx.getTradeStep?.() ?? { minSol: 0.0002 }) } as any;
      ...
      if ((bot.solBalance ?? 0) < need) {
        ...
        if (bot.posToken > 0) {
          ...
          const sellTok = roundTok(
            Math.max(0, Math.min(bot.posToken * (Number(risk.maxSellSliceTokPct) || 0.035), wantSol / Math.max(1e-12, priceNow))),
            decimals
          );
          if (sellTok > 0) {
    ...
    // adaptive slippage + priority fee
    const short = Math.abs(ctx.changeFast?.(12) || 0);
    const oneMin = Math.abs(ctx.change1m?.() || 0);
    const volScore = Math.max(short, oneMin);
    let lo = MIN_SLP_BPS, hi = MAX_SLP_BPS;
    if (volScore < 0.002) { lo = 30; hi = 60; } else if (volScore < 0.006) { lo = 50; hi = 90; } else { lo = 80; hi = 120; }
    ...
    const payload =
      side === "buy"
        ? { publicKey: kp.publicKey.toBase58(), action: "buy",  mint: ctx.mint, denominatedInSol: "true",
            amount: sizeSol, slippage: usedBps / 100, priorityFee: priorityFeeSol, pool: "auto" }
        : { publicKey: kp.publicKey.toBase58(), action: "sell", mint: ctx.mint, denominatedInSol: "false",
            amount: amountTok ?? roundTok(bot.posToken, decimals), slippage: usedBps / 100, priorityFee: priorityFeeSol, pool: "auto" };

    try {
      // sanity check (BUY): импакт и roundtrip
      const quoteFn = ctx.getJupiterQuote || getJupiterQuote;
      if (side === "buy") {
        ...
        try {
          ...
          if (out > 0 && Math.random() < RT_SAMPLE) {
            try {
              ...
              const maxRt = Math.max(0, Number(risk.maxRoundtripLoss ?? MAX_ROUNDTRIP_LOSS));
              if (isFinite(lossPct) && lossPct > maxRt) {
                warnDebounced(`skip BUY: roundtrip ${(lossPct*100).toFixed(1)}% > ${(maxRt*100).toFixed(1)}%`);
                return;
              }
              if (impact > 0.01) { (payload as any).__shrinkBuy = true; }
            } catch {}
          }
        } catch {}
      } // ← важно: закрываем if (side === "buy")
      ...
      for (let si = 0; si < slices; si++) {
        ...
        if (side === "buy") {
          ...
          const tnow = Date.now();
          if (tnow - minWindowStart >= 60_000) { minWindowStart = tnow; buysThisMin = 0; sellsThisMin = 0; notionalThisMin = 0; }
          ...
          // SELL: коридор + импакт на текущем срезе
          const pnow = Math.max(1e-12, ctx.price());
          const currTokVal = localPosToken * pnow;
          const totalLocal = Math.max(1e-9, currTokVal + localSol);
          const minTokValAfter = Math.max(0, Math.min(0.98, MIN_ALLOC + EPS_LOCAL) * totalLocal);
          ...
          const perTok = maxSellPerSlice > 0
            ? Math.min(maxSellPerSlice, remainingTok, maxSellTokLocal)
            : Math.min(remainingTok / Math.max(1, (slices - si)), maxSellTokLocal);

          let qty = roundTok(Math.max(0, perTok), decimals);
          if (qty <= 0) break;

          // импакт‑проверка для среза
          try {
            ...
            if (!isFinite(outSol) || outSol <= 0) { warnDebounced("skip SELL: illiquid route or out=0"); break; }

            const fair = qty * pnow;
            const thr  = Math.max(0, Math.min(0.2, Number(risk.maxImpact ?? MAX_SINGLE_TRADE_IMPACT)));
            const imp  = fair > 0 ? Math.max(0, 1 - outSol / fair) : 0;

            if (imp > thr) {
              const shrunk = roundTok(qty * 0.55, decimals);
              if (shrunk > 0 && shrunk < qty) qty = shrunk;
              else { warnDebounced(`skip SELL: impact ${(imp*100).toFixed(1)}% > ${(thr*100).toFixed(1)}%`); break; }
            }
          } catch { warnDebounced("skip SELL: quote failed"); break; }
      ...
      bot.last =
        side === "buy"
          ...
          : `sell ${roundTok(executedTok, decimals)} TOK @ slp=${ctx.slippageBps().toFixed(0)}bps`;
      ...
      // корректирующие nudges к коридору/цели
      try {
        const pnow = Math.max(1e-12, ctx.price());
  ...
  async function loop() {
    ...
    // отложенная небольшая продажа (fee‑буфер/паттерн)
    ...
    try {
      ...
      // защитный режим при большой просадке
      ...
      // emergency: нет SOL → продадим немного
