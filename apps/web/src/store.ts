  ...
  persist(
    (set, get) => ({
        ...
        for (const sender of bots) {
          ...
          for (let i = 0; i < txPerBot; i++) {
            try {
              ...
              const tx = new Transaction({ feePayer: fromPk, recentBlockhash: bh.blockhash }).add(ix)
              tx.sign(kp)
              const sig = await connection.sendRawTransaction(tx.serialize(), { skipPreflight: false })
              await connection.confirmTransaction({ signature: sig, ...bh }, 'confirmed')
              s.addLog('ok', `Warm-up ${sender.name} → ${receiver.name}: ${(lamports/LAMPORTS_PER_SOL).toFixed(6)} SOL (${i+1}/${txPerBot})`)
            } catch (e:any) {
      ...
      async startBot(id, connection) {
        ...
        const runnerLoader = (get().external.provider === 'pumpportal')
      ...
      // === Pump: создать токен и автопокупка ботами ===
      ...
      async buyAllBotsOnPump(connection, opts = {}) {
        const keep = Math.max(0.0005, opts.keepFeeSol ?? get().minFeeSol)
        const s = get()
      ...
      // === Pump: продать всё — боты → кошелёк → одна продажа
      async sellAllToWalletOnPump(connection, walletPubkey, _opts = {}) {
        ...
        // ensure ATA кошелька (кошелёк платит комиссию)
        try { await ensureWalletAta(connection as Connection, walletPubkey, s.tokenMint) } catch (e) { s.addLog('warn', `Ensure wallet ATA: ${String((e as any)?.message||e)}`) }

        // 1) собрать токены с ботов на кошелёк
        for (let i = 0; i < s.bots.length; i++) {
          const b = s.bots[i]
          try {
            const kp = getKeypair(b.keyId)
        ...
        // 2) продать все токены из кошелька одной транзакцией PumpPortal
        try {
          ...
          const vtx = await buildTradeTxPumpLocal({
            ...
            action: 'sell',
            mint: s.tokenMint,
            denominatedInSol: 'false',       // продаём кол-во токенов
